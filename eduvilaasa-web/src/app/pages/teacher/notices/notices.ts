...
@Component({
  ...
  template: `
    <div class="page-header"><div><h1>Notices</h1><p>Announcements from school management</p></div></div>
    @if (loading()) { <div class="card"><app-skeleton [lines]="4"/></div> }
    @else if (!items().length) { <div class="card"><div class="empty-state">No announcements yet.</div></div> }
    @else {
      @for (item of items(); track item.id) {
        <div class="card" style="margin-bottom:12px">
          ...
          <div style="font-size:11px;color:#666;margin-top:8px">By {{ item.created_by_name || 'Admin' }}</div>
        </div>
      }
    }
  `,
})
export class TeacherNoticesPage implements OnInit {
